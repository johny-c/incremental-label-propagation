...
class VarNeighborsUnLabeled(BaseExperiment):

    def __init__(self, n_neighbors_values, params, n_runs, isave=100):
        super(VarNeighborsUnLabeled, self).__init__(name='k_U', config=params,
                                                    isave=isave,
    ...
    def pre_single_run(self, X_run, y_run, mask_labeled, n_burn_in,
                       seed_run, X_test, y_test, n_run):
        params = self.config

        for n_neighbors in self.n_neighbors_values:
            params['graph']['n_neighbors_unlabeled'] = n_neighbors
            save_dir = os.path.join(self.top_dir,
                                    'k_U_' + str(n_neighbors))
...
if __name__ == '__main__':
    ...
    config_file = os.path.join(CONFIG_DIR, 'var_k_U.yml')
    ...
    N_NEIGHBORS_VALUES = config['graph']['n_neighbors_unlabeled'].copy()

    experiment = VarNeighborsUnLabeled(N_NEIGHBORS_VALUES, params=config,
                                     n_runs=args['n_runs'])
    if args['plot'] != '':
